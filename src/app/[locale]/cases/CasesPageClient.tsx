            ...
            >
              ...
              {selectedCase.images.map((img, idx) => (
                <div key={img} className={`absolute inset-0 transition-opacity duration-500 ${idx === modalSlide ? 'opacity-100' : 'opacity-0'}`}>
                  <Image src={img} alt={selectedCase.project ?? selectedCase.client} fill className="object-cover" priority={idx === 0} sizes="100vw" />
                </div>
              ))}
                  ...
                  {c.images.map((img, idx) => {
                    ...
                    return (
                      <div key={img} className={`absolute inset-0 transition-opacity duration-500 ${idx === activeSlide ? 'opacity-100' : 'opacity-0'}`}>
                        <Image src={img} alt={c.project ?? c.client} fill className="object-cover" draggable={false} loading="lazy" sizes="(max-width: 768px) 50vw, 25vw" />
                      </div>
                    );
                  })}
                </div>
